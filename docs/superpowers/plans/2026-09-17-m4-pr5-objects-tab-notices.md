...
- OpenNGC Type → kind: galaxy (`G`, `GPair`, `GTrpl`, `GGroup`), nebula (`Neb`, `EmN`, `RfN`, `HII`, `PN`, `SNR`, `DrkN`), cluster (`OCl`, `GCl`, `Cl+N`), other (everything else, including `*`, `**`, `*Ass`, `Nova`, `NonEx`). `names.json` becomes `{"aliases": [...], "kinds": {...}}`; the `kinds` map stores only galaxy/nebula/cluster entries (a missing key is `other`).
