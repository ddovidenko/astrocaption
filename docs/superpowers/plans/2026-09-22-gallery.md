...
- Public file URLs carry `?v=<exported_at>` and are served with `Cache-Control: public, no-cache`.
...
Cache header `public, no-cache`, so an unpublished image cannot outlive its unpublish in a cache; the URLs still carry `?v=<exported_at>` so the browser refreshes an `<img>` after a re-export.
