...
beforeEach(() => {
  ...
  vi.mocked(api.imageDefaultStyle).mockReset()
})
afterEach(() => {
  cleanup()
  vi.useRealTimers()
...
describe('StyleTab numbers', () => {
  ...
  it('the debounce waits the full 400 ms', async () => {
    state().load(doc())
    render(<StyleTab />)
    fireEvent.change(fontSize(), { target: { value: '30' } })
    await tick(399)
  ...
  it('an invalid keystroke cancels a prior valid pending value', async () => {
    state().load(doc())
    render(<StyleTab />)
    fireEvent.change(fontSize(), { target: { value: '50' } })
    fireEvent.change(fontSize(), { target: { value: '3' } })
    ...
    expect(fontSize().value).toBe('3')
    expect(fontSize().getAttribute('aria-invalid')).toBe('true')
  })
  it('a flush leaves no late duplicate', async () => {
    state().load(doc())
    render(<StyleTab />)
    fireEvent.change(fontSize(), { target: { value: '30' } })
    fireEvent.blur(fontSize())
    expect(state().style?.font_size).toBe(30)
    expect(state().undo).toHaveLength(1)
    const statusAfterBlur = state().save.status
    await tick(400)
    expect(state().undo).toHaveLength(1)
    expect(state().save.status).toBe(statusAfterBlur)
  })
})
